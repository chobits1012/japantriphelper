...
const TimelineEvent: React.FC<TimelineEventProps> = ({ event }) => {
  ...
  return (
    <div className={`relative pl-8 mb-8 last:mb-0 group`}>
      {/* Time Dot */}
      <div 
        className={`absolute -left-[11px] top-1 w-[22px] h-[22px] rounded-full border-[3px] z-10 bg-white dark:bg-slate-900 transition-all duration-300 flex items-center justify-center ${
          event.highlight 
      ...
      <div className="flex flex-col items-start w-full">
        <div className="flex items-center gap-3 mb-1">
          <span className="text-sm font-bold text-gray-400 font-sans tracking-wide dark:text-slate-500">
            {event.time}
          </span>
          {event.category && (
            <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-gray-100 border border-gray-200 text-[10px] font-bold text-gray-600 uppercase tracking-wider dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300">
        ...
        <div className="flex justify-between items-start w-full gap-4">
          <h4 className={`text-lg md:text-xl font-serif font-bold text-ink leading-tight dark:text-slate-100 ${event.highlight ? 'text-japan-blue underline decoration-japan-blue/20 decoration-2 underline-offset-4 dark:text-sky-400 dark:decoration-sky-500/30' : ''}`}>
            {event.title}
          </h4>
          ...
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            {(event.mapQuery || event.category === 'sightseeing' || event.category === 'food' || event.category === 'hotel') && (
              <button 
                ...
                className="p-1.5 hover:bg-gray-100 rounded-full text-gray-400 hover:text-japan-blue transition-colors dark:hover:bg-slate-800 dark:text-slate-500 dark:hover:text-sky-400"
        ...
        <div className="mt-2 text-sm md:text-base text-gray-600 leading-relaxed bg-white/60 backdrop-blur-sm px-4 py-3 rounded-lg shadow-sm border border-white/50 w-full md:w-auto hover:bg-white/80 transition-colors dark:bg-slate-800/50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800/80">
          {event.desc}
        </div>
        
        {/* Special Transport Badge */}
        {event.transport && (
          <div className="mt-2 inline-flex items-center gap-1.5 px-3 py-1 bg-[#8c7b6c] text-white text-xs rounded-full shadow-sm dark:bg-slate-700">
